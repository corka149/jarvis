defmodule JarvisWeb.ListLive.Form do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:list, list)
    |> assign(:form, to_form(Shopping.change_list(list)))
  end
  ...
  def handle_event("validate", %{"list" => list_params}, socket) do
    changeset = Shopping.change_list(socket.assigns.list, list_params)
  ...
  defp save_list(socket, :edit, list_params) do
    case Shopping.update_list(socket.assigns.list, list_params) do
      {:ok, list} ->
        {:noreply,
         socket
  ...
  defp return_path("index", _list), do: ~p"/shopping_lists"
  defp return_path("show", list), do: ~p"/shopping_lists/#{list}"
end
